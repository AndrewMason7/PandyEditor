...
extension EditorView {
    ...
    @objc internal func textDidChangeInternal(immediate: Bool = false) {
        ...
        if immediate.negated && (now - lastEditTime < minEditInterval) { return }
        lastEditTime = now
        ...
        let item = DispatchWorkItem { [weak self] in
            ...
            CrashGuard.onMainThread { [weak self] in
                ...
                // 6. Update Minimap (Lag Prevention)
                self.minimapView?.updateMinimap(with: self.text)
        ...
        let delay = immediate ? 0 : highlightDelay
        textProcessingQueue.asyncAfter(deadline: .now() + delay, execute: item)
    }
}
