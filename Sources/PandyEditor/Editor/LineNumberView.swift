...
internal class LineNumberView: UIView {
    ...
    init(textView: EditorSurface, fontSize: CGFloat = 14, theme: CodeEditorTheme = .oneDarkPro) {
    ...
    func invalidateLineCache() {
        ...
        guard let textView = textView, let text = textView.text else {
            resetCache()
            return
        }
    ...
    private func resetCache() {
        ...
        CrashGuard.onMainThread { [weak self] in self?.setNeedsDisplay() }
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        // BUG FIX: Limit buffer and intersect with content height to prevent drawing outside valid text range
        let bufferY: CGFloat = 100
        let contentRect = CGRect(x: 0, y: 0, width: CGFloat.max, height: textView.contentSize.height)
        let rangeRect = visibleRect.insetBy(dx: 0, dy: -bufferY).intersection(contentRect)
        ...
        layoutManager.enumerateLineFragments(forGlyphRange: validGlyphRange) { rect, _, _, fragmentGlyphRange, _ in
            ...
            if charStart == paragraphStart {
                if paragraphStart != lastDrawnParagraphStart {
                    ...
                    let rawY = rect.origin.y + textView.textContainerInset.top - textView.contentOffset.y + (rect.height - self.font.lineHeight) / 2.0
                    
                    // Pixel-Align for Retinal Clarity
                    let yPos = floor(rawY * screenScale) / screenScale
        ...
        // 4. Phantom Line Handling (Bulletproof)
        // If the text ends with a newline, the LayoutManager doesn't create a fragment
        // for the empty space after it. We must manually draw that line number.
        cacheLock.lock()
        let lastIndex = indices.last ?? 0
        let totalCount = indices.count
        cacheLock.unlock()
        
        if lastIndex > lastDrawnParagraphStart {
            let extraRect = layoutManager.extraLineFragmentRect
            if extraRect.height > 0 {
                let yPos = extraRect.origin.y + textView.textContainerInset.top - textView.contentOffset.y + (extraRect.height - self.font.lineHeight) / 2.0
                // Only draw if within visible vertical bounds (with buffer)
                if yPos > -50 && yPos < bounds.height + 50 {
                    self.drawNumber(totalCount, atY: yPos, width: self.bounds.width)
                }
            }
        }
    }
