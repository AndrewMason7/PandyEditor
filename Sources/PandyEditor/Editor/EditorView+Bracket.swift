...
extension EditorView {
    ...
    func updateBracketMatching() {
        ...
        performSafeUpdate {
            // LAG PREVENTION: Debouncing (Wait 50ms to ensure cursor has settled)
            self.bracketMatchingWorkItem?.cancel()
            
            let item = DispatchWorkItem { [weak self] in
                self?.performBracketMatching()
            }
            self.bracketMatchingWorkItem = item
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.05, execute: item)
        }
    }
    
    private func performBracketMatching() {
        // RE-VALIDATE Safety Guards after debounce
        guard showBracketMatching else { return }
