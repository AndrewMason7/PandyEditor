...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Tests-20%20Passing-brightgreen?style=flat-square" alt="20 Tests" />
</p>
...
*   **Branch Dependency**: FiveKit uses `branch: "main"` (no semantic version tags available in upstream repository).
...
└── Tests/PandyEditorTests/
    └── PandyEditorTests.swift          # 20 Unit Tests
```
...
**Critical Fix:**
- 🔧 Fixed FiveKit dependency resolution failure - changed from `from: "1.0.0"` to `branch: "main"` since FiveKit repository has no semantic version tags

### v1.0.1 (December 2025)

**Bug Fixes:**
- ✅ Added missing `incrementTextVersion()` and `currentTextVersion()` methods for atomic versioning
- ✅ Added missing `showLineNumbers` property to `EditorView`
- ✅ Fixed `updateFontSize()` method signature to accept optional parameter
- ✅ Fixed missing `FiveKit` imports in extension files
- ✅ Fixed `isNotEmpty` usage (not in FoundationPlus) → replaced with `isEmpty.negated`
- ✅ Fixed `setLanguage()` not updating `currentLanguage` state

**Improvements:**
- 🔧 Complete architectural audit and code cleanup
- 🔧 All files now properly import FiveKit for FoundationPlus extensions
- 🔧 Thread-safe version management with `NSLock` protection

---

## 🤝 Contributing

See [CONTRIBUTING.md](CONTRIBUTING.md) for the **FiveKit Compliance Workflow** including:

- The **Safety Quadruple** pattern for UI methods
- **View Diffing** for lag prevention
- **CrashGuard** utilities usage
